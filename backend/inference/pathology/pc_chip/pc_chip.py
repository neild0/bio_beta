...
class PC_CHiP(Model):
    def __init__(self, version: str = "og"):
        ...
        def model(images):
            with slim.arg_scope(args):
                return PC_CHiP_arch(
                    images, NUM_CLASSES, is_training=False, version=version
                )

        self.model = model
        curDir = pathlib.Path(__file__).parent.absolute()
        checkpoint_paths = {
            "og": f"{curDir}/model/Retrained_Inception_v4",
        ...
        if not os.path.exists(checkpoint_paths[version]):
            ...
            subprocess.call(
                [f"{curDir}/setup.sh", curDir, version],
                stdout=sys.stdout,
    ...
    def predict(self, image_dataset, tissue_cat=None, separate=False):
        ...
        if type(image_dataset) != list:
            image_path_list = [
                f"{image_dataset}/{x}" for x in os.listdir(image_dataset)
            ]
        else:
            image_path_list = image_dataset
        for image_path in image_path_list:
            proc_image = pre_proc.preProc(image_path)
            im_pred = self.model.run(
                self._probabilities, feed_dict={self._image_data: proc_image}
            )
