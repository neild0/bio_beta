...
class Proc_PC_CHIP(PreProc):
    ...
    def procSlide(self, slide_path, dataset_path):
        ...
        if (
            str(slide.properties.values.__self__.get("tiff.ImageDescription")).split(
                "|"
            )[1]
            == "AppMag = 40"
        ):
        ...
        for x in range(1, w, seq):
            for y in range(1, h, seq):
                ...
                if counts[np.argwhere(unique <= 20)].sum() < 512 * 512 * 0.6:
                    img111.save(
                        f"{dataset_path}/{slide_name}_x{x}_y{y}.jpg",
                        "JPEG",
                        optimize=True,
                        quality=94,
                    )
    ...
    def postProc(self, raw_output):
        if self.tissue_cat is not None:
            if self.tissue_cat in self.cat_pred_dict:
                normal_ix, tumor_ix = (
                    self.cat_pred_dict[self.tissue_cat][0],
                    self.cat_pred_dict[self.tissue_cat][1],
                )
                all_preds = {
                    self.pred_dict[normal_ix]: raw_output[0][normal_ix],
            ...
            all_preds = {
                self.pred_dict[label_ix]: pred_val
                for label_ix, pred_val in enumerate(raw_output[0])
            }
            return [max_pred, all_preds]
