...
TRANSFORM_PATH = (
    "data/ref/enformer.finetuned.SAD.robustscaler-PCA500-robustscaler.transform.pkl"
)
...
class Enformer(Model):
    ...
    def predict_expression(self, data: str, chrom: int, start: int, end: int) -> dict:
        ...
        sequence_one_hot = self.one_hot_encode(
    ...
    def variant_scoring(
        self, data: str, chrom: str, pos: int, ref_base: str, var_base: str, var_id: str
    ) -> dict:
        variant = kipoiseq.Variant(chrom, pos, ref_base, var_base, var_id)
        fasta_extractor = FastaStringExtractor(data)

        interval = kipoiseq.Interval(
            variant.chrom, variant.start, variant.start
        ...
        reference_prediction = self.predict(self.one_hot_encode(reference)[np.newaxis])[
            self.species
        ][0]
        alternate_prediction = self.predict(self.one_hot_encode(alternate)[np.newaxis])[
            self.species
        ][0]
    ...
    with _open(vcf_file) as f:
        for line in f:
            ...
            for alt in alt_list.split(","):
                yield kipoiseq.dataclasses.Variant(
                    chrom=chrom, pos=pos, ref=ref, alt=alt, id=id
                )


def variant_centered_sequences(
    data, vcf_file, sequence_length, gzipped=False, chr_prefix=""
):
    seq_extractor = kipoiseq.extractors.VariantSeqExtractor(
    ...
    for variant in variant_generator(vcf_file, gzipped=gzipped):
        ...
        yield {
            "inputs": {
                "ref": Enformer.one_hot_encode(reference),
