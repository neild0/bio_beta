      ...
      .then((res) => {
        ...
        for (let i = 0; i < 5; i++) {
          contentList.LCL.push(
            <Row span={30}>
              <Progress
                percent={Number((sortable[i].score * 100).toFixed(0))}
                status="active"
              />
              {sortable[i].label}
            </Row>
        ...
        for (let i = 0; i < sortable2.length; i++) {
          contentList.MS.push(
            <Row span={30}>
              <Progress
                percent={Number((sortable2[i].score * 100).toFixed(0))}
                status="active"
              />
              {sortable2[i].label}
            </Row>
        ...
        contentList.SS3.push(
          <>
            {" "}
            <Text style={{ color: "#5f59ff" }}> Alpha-Helix - </Text>
        ...
        for (let i = 1; i < res.data.SS3.length - 1; i++) {
          contentList.SS3.push(
            <Text style={{ color: aa_colors[res.data.SS3[i].entity] }}>
              {res.data.SS3[i].word}
            </Text>
          );
        }
