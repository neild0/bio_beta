...
class ImageResults extends React.Component {
    ...
    runExperiment() {
        this.setState({ contentList: {...this.state.contentList, LCL: 'loading' }});
        axios
            .get("http://192.168.1.202:3334/api/site_enformer", { params: { chrom: this.state.chrom, start: this.state.start, end: this.state.end } })
            .then(res => {
                ...
                this.setState({ contentList: {...this.state.contentList, LCL: content }});
...
// add range check
                ...
                <Row gutter={16}>
                    <Col span={4}>
                        ...
                        <InputNumber
                            ...
                            onChange={value => {this.setState({chrom: value})}}
                        />
                    ...
                    <Col span={14}>
                        <Slider
                            ...
                            onChange={values => {this.setState({start: values[0], end: values[1]})}}
        ...
        return (<>
                ...
                {this.state.contentList[this.state.key] == null ? null :
                    (this.state.contentList[this.state.key] == 'loading') ?
                    <Card style={{width: '100%', textAlign: 'center'}} loading={true}>
                    </Card> : this.state.contentList[this.state.key]}
