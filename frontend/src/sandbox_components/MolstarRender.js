...
const MolstarRender = (props) => {
    ...
    function loadPdb(pdb, options) {
        const params = DownloadStructure.createDefaultParams(plugin.state.data.root.obj, this.plugin);
        const provider = plugin.config.get(PluginConfig.Download.DefaultPdbProvider);
        return plugin.runTask(this.plugin.state.data.applyAction(DownloadStructure, {
            source: {
                name: 'pdb',
                params: {
                    provider: {
                    ...
                    options: { ...params.source.params.options, representationParams: options?.representationParams},
                }
            }
        }));
    }
